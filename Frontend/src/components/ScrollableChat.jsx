...
const ScrollableChat = ({ messages }) => {
  ...
  return (
    <div className="flex flex-col space-y-1 p-4 overflow-y-auto h-full custom-scrollbar">
      {messages &&
        messages.map((m, i) => (
          <div className="flex items-end group" key={m._id}>
              ...
              (isSameSender(messages, m, i, user._id) || isLastMessage(messages, i, user._id)) && (
                <div className="relative mr-3 mb-1">
                  ...
                  >
                    {m.sender.pic ? (
                      <img
                        src={m.sender.pic || "/placeholder.svg"}
                        alt={m.sender.name}
            ...
            <div className="flex flex-col max-w-xs lg:max-w-md">
              ...
              <div
                className={`px-4 py-3 rounded-2xl shadow-sm transition-all duration-200 ${
                  m.sender._id === user._id
                    ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white ml-auto rounded-br-md"
              ...
              >
                ...
                {m.sender._id !== user._id && !isSameUser(messages, m, i, user._id) && (
                  <div className="text-xs font-semibold text-blue-600 mb-1">{m.sender.name}</div>
                )}
